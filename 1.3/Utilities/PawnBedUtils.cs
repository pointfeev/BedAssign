...
{
    public static bool CanBeUsed(this Pawn pawn) => pawn is { ownership: { }, Faction.IsPlayer: true, IsFreeNonSlaveColonist: true, def.race.Humanlike: true };
    ...
    public static bool CanBeUsedEver(this Building_Bed bed)
        => bed is not null && !bed.IsHospitalityGuestBed() && !bed.Medical && bed.ForColonists && bed.def.building.bed_humanlike;
