...
public static class PawnBedUtils
{
    public static bool CanBeUsed(this Pawn pawn)
        => pawn is { ownership: not null, Faction.IsPlayer: true, IsFreeNonSlaveColonist: true, def.race.Humanlike: true };
    ...
    public static bool CanBeUsedEver(this Building_Bed bed)
        => bed is not null && (!ModSettings.IgnoreSleepingSpots || bed.def.defName != "SleepingSpot" && bed.def.defName != "DoubleSleepingSpot")
                           && !bed.IsHospitalityGuestBed() && !bed.Medical && bed.ForColonists && bed.def.building.bed_humanlike;
